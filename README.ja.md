...
- **包括的なセキュリティ強化と認証保護**：
  - **認証境界の厳格化**：未検証 JWT および URL クエリトークン認証の廃止；MAC/IP 情報単独でのアクセス権限付与の廃止；セッション Cookie への `HttpOnly` および `SameSite` 属性の適用、同一オリジン CSRF、CORS、信頼できるプロキシ検証の導入。
  ...
  - **入力およびコンテンツ防御**：画像 MIME の偽装、SVG XSS 注入、巨大画像展開爆弾攻撃を包括的に修正；管理画面の蓄積型 XSS を修正；API/MCP リクエストボディ、記事タイトル、本文、メタデータのサイズ上限を制限し、空の記事や空の返信を禁止。
  - **ストレージコアの並行性安全性**：Store 内部のデータ競合、ミューテックスの誤用、構造体コピーの問題を修正し、高並行環境におけるデータ整合性を向上。
