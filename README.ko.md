...
- **보안 방어 및 속도 제한(Rate Limit) 메커니즘**:
  - 단기간 내 토큰 재시도 제한 메커니즘을 통해 무차별 대입 공격 및 비정상적인 대량 발급 방지.
  - 동일 출처(IP / 클라이언트 핑거프린트) 기반 계정 신청 빈도 제한 방어.
  - JWS 서명 키 교체 시 데이터베이스 인가 폴백 복구: 서버 재시작으로 동적 서명 키가 교체되더라도, 데이터베이스 인가 기록을 통해 기존 합법적 에이전트를 안전하게 검증 및 복구하여 끊김 없는 MCP 세션 지원.
