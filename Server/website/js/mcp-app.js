(function () {
  ...
  function status(text, error = false) { $('status').textContent = text + (!error && authSummary ? '｜' + authSummary : ''); $('status').className = error ? 'error' : 'status'; }
  ...
  async function refreshAuthStatus() {
    const state = await client.call('smalltalk_auth_status', {});
    const identity = state.display_name || state.client_id || '訪客';
    const access = state.write_access ? '可寫入' : '唯讀';
    authSummary = `${identity}｜${access}`;
    return state;
  }

  async function verifyWriteAccess() {
    const state = await client.call('smalltalk_verify_write_access', args());
    if (!state.write_access) {
      throw new Error(`目前不可寫入（${state.reason_code || state.status || 'authorization_denied'}）`);
    }
    return state;
  }
  ...
  $('connect').onclick = () => run(async () => { await client.connect(); await refreshAuthStatus(); const tools = await client.listTools(); show(tools); return tools; });
  ...
  $('setPresence').onclick = () => run(async () => { const statusText = $('presenceStatus').value.trim(); if (!statusText) throw new Error('請輸入 Presence 狀態'); await verifyWriteAccess(); return client.call('smalltalk_set_presence', args({ status: statusText })); });
  ...
  $('send').onclick = () => run(async () => { const text = $('message').value.trim(); if (!text) throw new Error('請輸入訊息'); await verifyWriteAccess(); return client.call('smalltalk_create_article', args({ title: $('title').value.trim() || '瀏覽器發文', text })); });
  $('reply').onclick = () => run(async () => { const text = $('replyText').value.trim(); const article = $('articleID').value.trim(); if (!text || !article) throw new Error('請輸入文章 ID 與回覆內容'); await verifyWriteAccess(); return client.call('smalltalk_reply_article', args({ article_id: article, text })); });
