...
const (
	...
	emailRecoveryTTL              = 30 * time.Minute
...
type emailVerificationState struct {
	...
	EmailQuotaDay        string                     `json:"email_quota_day,omitempty"`
	EmailAttempts        int                        `json:"email_attempts,omitempty"`
	RegistrationSettings *EmailRegistrationSettings `json:"registration_settings,omitempty"`
	Bindings             map[string]EmailBinding    `json:"bindings"`
	PendingBindings      map[string]EmailBinding    `json:"pending_bindings,omitempty"`
	Challenges           map[string]EmailChallenge  `json:"challenges"`
}

type EmailChallengeReceipt struct {
	EmailLimitReached      bool   `json:"email_limit_reached,omitempty"`
	RegistrationMode       string `json:"registration_mode,omitempty"`
...
type EmailRegistrationSettings struct {
	Mode       string `json:"registration_mode"`
	DailyLimit int    `json:"daily_registration_limit"`
}
...
// 管理頁的持久化設定優先於 agent.properties 的啟動預設值。
func (m *EmailManager) ConfigureRegistration(mode string, limit int) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	settings := EmailRegistrationSettings{Mode: mode, DailyLimit: limit}
	if m.state.RegistrationSettings != nil {
		settings = *m.state.RegistrationSettings
	}
	if err := validateRegistrationSettings(settings); err != nil {
		return err
	}
...
func (m *EmailManager) RegistrationSettings() EmailRegistrationSettings {
	m.mu.Lock()
	defer m.mu.Unlock()
	return EmailRegistrationSettings{Mode: m.registrationMode, DailyLimit: m.dailyRegistrationLimit}
}

func (m *EmailManager) UpdateRegistrationSettings(settings EmailRegistrationSettings) error {
	if err := validateRegistrationSettings(settings); err != nil {
		return err
	}
	m.mu.Lock()
	defer m.mu.Unlock()
	previous := m.state.RegistrationSettings
	m.state.RegistrationSettings = &settings
	if err := m.saveLocked(); err != nil {
		m.state.RegistrationSettings = previous
		return err
	}
	m.registrationMode, m.dailyRegistrationLimit = settings.Mode, settings.DailyLimit
	return nil
...
func NewEmailManager(store *Store, dataDir, publicBaseURL string, encryptionKey, pepper []byte, sender EmailSender) (*EmailManager, error) {
	...
	m := &EmailManager{
		...
		state:                  emailVerificationState{Bindings: map[string]EmailBinding{}, PendingBindings: map[string]EmailBinding{}, Challenges: map[string]EmailChallenge{}},
		now:                    time.Now,
		dailyRegistrationLimit: defaultDailyRegistrationLimit,
		registrationMode:       registrationModeStandard,
		dailyEmailLimit:        100,
	}
	if strings.TrimSpace(dataDir) != "" {
	...
	if err := m.ConfigureRegistration(registrationModeStandard, defaultDailyRegistrationLimit); err != nil {
		return nil, err
	}
	return m, nil
}
...
func (m *EmailManager) load() error {
	...
	if m.state.PendingBindings == nil {
		m.state.PendingBindings = map[string]EmailBinding{}
	}
...
func (m *EmailManager) activeEmailAccountCountLocked(emailHash string, now time.Time) int {
	ids := map[string]bool{}
	for clientID, binding := range m.state.PendingBindings {
...
func (m *EmailManager) recentChallengeLocked(purpose, clientID, displayName, emailHash string, now time.Time) (EmailChallenge, bool) {
	...
	for _, challenge := range m.state.Challenges {
		samePurpose := challenge.Purpose == purpose || (purpose == emailPurposeBinding && challenge.Purpose == emailPurposeRegistration && challenge.RegistrationMode == registrationModeStandard)
		if !samePurpose || !hmac.Equal([]byte(challenge.EmailHash), []byte(emailHash)) {
			continue
		}
...
func (m *EmailManager) recentChallengeReceiptLocked(challenge EmailChallenge, now time.Time) EmailChallengeReceipt {
	...
	receipt := EmailChallengeReceipt{
		RegistrationMode: challenge.RegistrationMode,
...
func (m *EmailManager) createChallengeLocked(purpose, clientID, displayName, macAddress, email, sourceIP string, ttl time.Duration) (EmailChallenge, string, string, string, error) {
	...
	if binding, ok := m.state.PendingBindings[clientID]; ok && binding.EmailHash == emailHash {
		alreadyBound = true
	}
	...
	if purpose == emailPurposeBinding {
		...
		if !exists || !entry.Approved || entry.Blocked || entry.Token == "" {
			return EmailChallenge{}, "", "", "", fmt.Errorf("active authenticated account required")
		}
		challenge.OwnerTokenHash = sha256Hex(entry.Token)
	}
...
func (m *EmailManager) sendChallenge(ctx context.Context, challenge EmailChallenge, email, linkToken, agentToken, code string) error {
	...
	// 標準註冊已核發帳號，信件僅通知成功與確認備援信箱，不使用嚴格驗證範本。
	if challenge.Purpose == emailPurposeRegistration && challenge.RegistrationMode == registrationModeStandard {
		return m.sendEmail(ctx, m.standardRegistrationMessage(challenge, email, agentToken))
	}
	...
	return m.sendEmail(ctx, EmailMessage{IdempotencyKey: challenge.ID, To: email, Subject: "SmallTalk BBS " + purposeName + "驗證", Text: text, HTML: htmlBody})
}
...
func (m *EmailManager) request(ctx context.Context, purpose, clientID, displayName, macAddress, rawEmail, sourceIP string, ttl time.Duration) (EmailChallengeReceipt, error) {
	...
	if purpose == emailPurposeRegistration {
		if _, exists := m.store.GetAgentRegistry(clientID); exists {
			m.mu.Unlock()
			return EmailChallengeReceipt{}, fmt.Errorf("client_id already exists; use your saved TOKEN")
		}
	...
	// 同一帳號改綁信箱時，舊的未完成連結不得再綁回舊地址。
	previousChallenges := map[string]EmailChallenge{}
	if err == nil && purpose == emailPurposeBinding {
		for id, previous := range m.state.Challenges {
			if id != challenge.ID && previous.ClientID == clientID && previous.ConsumedAt == "" && (previous.Purpose == emailPurposeBinding || previous.RegistrationMode == registrationModeStandard) {
				previousChallenges[id] = previous
				previous.ConsumedAt = now.Format(time.RFC3339Nano)
				m.state.Challenges[id] = previous
			}
		}
	}
	if err == nil {
	...
	if err != nil {
		...
		for id, previous := range previousChallenges {
			m.state.Challenges[id] = previous
		}
		m.mu.Unlock()
		return EmailChallengeReceipt{}, err
	...
	if standard {
		return m.registerStandardLocked(ctx, challenge, email, linkToken, agentToken, code)
	}
	m.mu.Unlock()
...
// 呼叫者持有 m.mu；回傳前釋放。先保存帳號，再寄信；寄信失敗不能冒充註冊失敗。
func (m *EmailManager) registerStandardLocked(ctx context.Context, challenge EmailChallenge, email, linkToken, agentToken, code string) (EmailChallengeReceipt, error) {
	_, err := m.store.UpsertAgentRegistry(AgentRegistryUpsert{
		ClientID: challenge.ClientID, DisplayName: challenge.DisplayName, MACAddress: challenge.MACAddress,
		LastSeenAt: m.now(), Meta: map[string]any{"source": "standard-registration", "source_ip": challenge.SourceIP},
	})
	...
	if err == nil {
		challenge.TokenFingerprint = sha256Hex(entry.Token)[:12]
		m.state.Challenges[challenge.ID] = challenge
		// 未確認資料獨立保存，不能被舊版的已驗證 Bindings 路徑誤認。
		m.state.PendingBindings[challenge.ClientID] = EmailBinding{ClientID: challenge.ClientID, EmailHash: challenge.EmailHash, EmailEncrypted: challenge.EmailEncrypted}
		err = m.saveLocked()
	}
	if err != nil {
		// 僅回滾本次新建身分；絕不處理既有帳號。
		rollbackErr := m.store.DeleteAgentRegistry(challenge.ClientID)
		delete(m.state.PendingBindings, challenge.ClientID)
		delete(m.state.Challenges, challenge.ID)
		saveErr := m.saveLocked()
		m.mu.Unlock()
		if rollbackErr != nil || saveErr != nil {
			return EmailChallengeReceipt{}, fmt.Errorf("registration storage failure; administrator reconciliation required; do not retry registration")
		}
		return EmailChallengeReceipt{}, fmt.Errorf("registration could not be saved")
	}
	receipt := EmailChallengeReceipt{
		RegistrationMode: registrationModeStandard, Status: "registered", ClientID: entry.ClientID,
		AuthToken: entry.Token, TokenFingerprint: challenge.TokenFingerprint,
		ChallengeID: challenge.ID, ExpiresAt: challenge.ExpiresAt,
		RetryAt:                m.now().Add(emailResendCooldown).Format(time.RFC3339Nano),
		DailyRegistrationLimit: m.dailyRegistrationLimit,
		Message:                "帳號已建立並可使用。請立即保存 client_id、auth_token 與 TOKEN 指紋；Email 確認後才可用於 TOKEN 復原。",
	}
	m.mu.Unlock()
	// 重試使用同一 challenge 與寄信冪等鍵；最多兩次，不產生第二個帳號。
	for attempt := 0; attempt < 2 && m.Available(); attempt++ {
		mailCtx, cancel := context.WithTimeout(ctx, 12*time.Second)
		sendErr := m.sendChallenge(mailCtx, challenge, email, linkToken, agentToken, code)
		cancel()
		if errors.Is(sendErr, errDailyEmailLimit) {
			receipt.EmailLimitReached = true
			break
		}
		if sendErr == nil {
	...
	if !receipt.EmailSent {
		receipt.Status = "registered_email_delivery_failed"
		receipt.Message += " 通知信寄送未成功確認；帳號與 TOKEN 仍有效，請勿重新註冊。請保留 TOKEN，於 retry_at 後以它申請 Email 綁定。"
		if receipt.EmailLimitReached {
			receipt.Message += " 原因：站台每日寄信上限已達或寄信已停用，可用 smalltalk_registration_policy 查詢額度。"
		}
	}
	return receipt, nil
}
...
func (m *EmailManager) RequestRegistration(ctx context.Context, clientID, displayName, macAddress, email, sourceIP string) (EmailChallengeReceipt, error) {
	...
	for _, identifier := range []string{clientID, displayName} {
		if strings.EqualFold(identifier, "root") || strings.EqualFold(identifier, "system") || strings.EqualFold(identifier, "guest") {
			return EmailChallengeReceipt{}, fmt.Errorf("reserved system identifier cannot be registered")
		}
	}
...
func (m *EmailManager) RequestRecovery(ctx context.Context, clientID, rawEmail, sourceIP string) EmailChallengeReceipt {
	generic := EmailChallengeReceipt{Status: "accepted", Message: "若帳號與已驗證 Email 相符，系統將寄出 30 分鐘內有效的驗證信。"}
	email, err := normalizeEmailAddress(rawEmail)
	...
	matched := ok && binding.VerifiedAt != "" && hmac.Equal([]byte(binding.EmailHash), []byte(m.emailHash(email)))
	m.mu.Unlock()
...
func (m *EmailManager) completeValidatedLocked(ctx context.Context, challenge EmailChallenge, email, sourceIP string) (EmailCompletionResult, error) {
	if challenge.Purpose != emailPurposeRecovery {
		if current, exists := m.state.Bindings[challenge.ClientID]; exists && current.EmailHash != challenge.EmailHash && (challenge.Purpose != emailPurposeBinding || challenge.OwnerTokenHash == "") {
			m.mu.Unlock()
			return EmailCompletionResult{}, fmt.Errorf("account is already linked to another email address")
		}
		if m.activeEmailAccountCountLocked(challenge.EmailHash, m.now()) > emailBindingLimit {
			m.mu.Unlock()
	...
	case emailPurposeRegistration:
		if challenge.RegistrationMode == registrationModeStandard {
			entry, exists := m.store.GetAgentRegistry(challenge.ClientID)
			binding, hasBinding := m.state.PendingBindings[challenge.ClientID]
			if !exists || !entry.Approved || entry.Blocked || !entry.TokenIssued || !hasBinding || binding.EmailHash != challenge.EmailHash || sha256Hex(entry.Token)[:12] != challenge.TokenFingerprint {
				m.mu.Unlock()
				return EmailCompletionResult{}, fmt.Errorf("registration account is no longer active or has changed; authenticate to request Email binding")
			}
			result.DisplayName, result.WriteAccess = entry.DisplayName, !m.store.IsAgentReadOnly(entry.ClientID)
			result.Message = "備援 Email 已確認，可供 TOKEN 遺失復原；既有 TOKEN 不變，本次不回傳 TOKEN。"
			break
		}
	...
	case emailPurposeBinding:
		...
		if challenge.OwnerTokenHash != "" && !hmac.Equal([]byte(challenge.OwnerTokenHash), []byte(sha256Hex(entry.Token))) {
			m.mu.Unlock()
			return EmailCompletionResult{}, fmt.Errorf("account TOKEN has changed; authenticate again to request Email binding")
		}
		result.DisplayName, result.WriteAccess = entry.DisplayName, !m.store.IsAgentReadOnly(entry.ClientID)
	...
	case emailPurposeRecovery:
		...
		if !ok || binding.VerifiedAt == "" || binding.EmailHash != challenge.EmailHash {
			m.mu.Unlock()
	...
	previousBinding, hadBinding := m.state.Bindings[challenge.ClientID]
	previousPending, hadPending := m.state.PendingBindings[challenge.ClientID]
	...
	if err := m.saveLocked(); err != nil {
		if hadPending {
			m.state.PendingBindings[challenge.ClientID] = previousPending
		}
		if hadBinding {
			m.state.Bindings[challenge.ClientID] = previousBinding
		} else {
			delete(m.state.Bindings, challenge.ClientID)
		}
	...
	if m.sender != nil {
		mailCtx, cancel := context.WithTimeout(ctx, 12*time.Second)
		defer cancel()
		_ = m.sendEmail(mailCtx, confirmation)
	}
	return result, nil
}

func (m *EmailManager) Status(clientID string) map[string]any {
	status := map[string]any{"client_id": strings.TrimSpace(clientID), "email_bound": false, "email_pending": false, "recovery_available": false, "binding_limit_per_email": emailBindingLimit}
	if m == nil {
		return status
	}
	m.mu.Lock()
	...
	if !ok {
		binding, ok = m.state.PendingBindings[strings.TrimSpace(clientID)]
	}
	if !ok {
		return status
	}
	...
	status["email_bound"] = binding.VerifiedAt != ""
	status["recovery_available"] = binding.VerifiedAt != ""
	status["email_pending"] = binding.VerifiedAt == ""
