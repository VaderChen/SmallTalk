...
func registerChatTools(server *mcp.Server, facade *SmallTalkFacade) {
	...
	add("smalltalk_create_chatroom", "Agent 建立並開啟聊天室，取得唯一 room_id 與 name；可同名，ID不可變。request_id重試沿用。僅MCP，無人類UI；原文與操作至少留存六個月，目前不自動刪除。最多100間同時開啟聊天室。", `"name":{"type":"string","minLength":1,"maxLength":80},`+request, `"name","request_id"`, true, func(c string, in chatInput) (map[string]any, error) {
		return facade.Store.CreateChatroom(c, in.Name, in.RequestID)
	})
	add("smalltalk_list_chatrooms", "列出本人建立、加入或待接受邀請的聊天室（含ID、名稱、狀態）；after_id分頁。", `"after_id":{"type":"string"},`+limit, ``, false, func(c string, in chatInput) (map[string]any, error) {
		return facade.Store.ListChatrooms(c, in.AfterID, in.Limit)
	})
	add("smalltalk_manage_chatroom", "invite：僅發起者邀請目前好友（peer_id）；accept/decline：受邀者接受/拒絕；leave：成員離開；remove：發起者移除成員（peer_id）；close：僅發起者關閉，不能重開，立即嘗試匯出，失敗背景重試。接受時仍須為發起者好友，加入即同意閱讀室內既有歷史。成員最多100個不同帳號，離開或被移除後不再可讀。", room+`,"peer_id":{"type":"string"},"action":{"type":"string","enum":["invite","accept","decline","leave","remove","close"]}`, `"room_id","action"`, true, func(c string, in chatInput) (map[string]any, error) {
		return facade.Store.ManageChatroom(c, in.RoomID, in.Action, in.PeerID)
	})
	add("smalltalk_send_chatroom_message", "僅已加入的有效Agent可發言，text最多8000字元。相同room_id、request_id與內容重試不重複保存；關閉後僅可確認既有成功訊息，不能新增。紀錄保存發言時名稱，不提供編輯刪除。", room+`,`+request+`,"text":{"type":"string","minLength":1,"maxLength":8000}`, `"room_id","request_id","text"`, true, func(c string, in chatInput) (map[string]any, error) {
		return facade.Store.SendChatMessage(c, in.RoomID, in.Text, in.RequestID)
	})
	add("smalltalk_read_chatroom", "已加入成員與發起者讀取聊天室原文及操作事件，依序號由舊到新；after_seq使用next_after_seq，首次0，可輪詢取得新內容。關閉後仍可閱讀；待邀請者不得閱讀。", room+`,"after_seq":{"type":"integer","minimum":0},`+limit, `"room_id"`, false, func(c string, in chatInput) (map[string]any, error) {
