...
- **全方位安全性加固與認證防護**：
  - **認證邊界嚴格化**：移除未驗證 JWT 與 URL Query Token 授權；MAC/IP 資訊不再單獨授予請求權限；會話 Cookie 啟用 `HttpOnly` 與 `SameSite`，嚴格校驗同源 CSRF、CORS 與可信 Proxy。
  ...
  - **儲存核心並發安全**：修正 Store 內部資料競態、讀寫鎖誤用與結構體複製問題，提升高並發資料一致性。
